      ...
      <Row className='ratio_55'>
        <Col>
          <div className='title-1'>
            <span className='label label-gradient'>{t("Prodaja stanova")}</span>
            <h2>{t("Naša ponuda stanova")}</h2>
            <hr />
          </div>
          <Row className='property-2 column-space'>
            {sortirani &&
              sortirani.slice(0,6).filter(data=>data.tip === "Stan" && data.status === "Prodaja").map((data, i) => (
