...
const RangeInputFields = ({ label, name, filterValues, setFilterValues, min, max, sm, lg }) => {
  ...
  const formatNumber = (number) => {
    return number.toLocaleString();  // Formatiranje broja sa zarezima
  };
      ...
      <NoSsr>
        <FormGroup>
          <div className="price-range">
            <Label>
              {t(label)} : {label === "Cijena" && `${symbol}`} 
              {label === "Površina" ? formatNumber(povrsina[0]) : formatNumber(cijena[0] * currencyValue)} - 
              {label === "Cijena" && `${symbol}`}{" "}
              {label === "Površina" ? formatNumber(povrsina[1]) : formatNumber(cijena[1] * currencyValue)} 
              {label === "Cijena" || "m2"}
            </Label>
