    ...
    <section className="p-0 ratio3_2">
      <Container fluid={true}>
        <div className="zoom-image-box zoom-gallery-multiple">
          <Row>
            <Gallery>
              <Item original={`/slike/${singleData?.slika}`|| "/assets/images/property/4.jpg"} width="1000" height="600">
                {({ ref, open }) => (
                  <Col md="6" className=" p-0" onClick={open}>
                    <div ref={ref}>
                      <Img src={id ? `/slike/${singleData?.slika}`  : "/assets/images/property/4.jpg"} className="bg-img" alt="" />
                    </div>
                  </Col>
                )}
              </Item>
