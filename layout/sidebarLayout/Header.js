...
const Header = ({ grid, mapView, mapModal, gridBar, tabHeader, title, AdvancedSearchShow, productCount, setMapModal, gridDispatch }) => {
  ...
  const { sortirajPrema, statusNekretnine } = useSelector((state) => state.inputsReducer);
      ...
      <div className="top-panel">
        {tabHeader ? (
          <div className="filters respon-filter-content filter-button-group">
            <ul>
              <li className={statusNekretnine === "Status Nekretnine" || !statusNekretnine?.length ? "active" : ""} onClick={() => dispatch({ type: "statusNekretnine", payload: "Status Nekretnine" })}>
                <span>{t("Sve nekretnine")}</span>
              </li>
              <li className={statusNekretnine === "Prodaja" ? "active" : ""} onClick={() => dispatch({ type: "statusNekretnine", payload: "Prodaja" })}>
                <span>{t("Prodaja")}</span>
              </li>
              <li className={statusNekretnine === "Iznajmljivanje" ? "active" : ""} onClick={() => dispatch({ type: "statusNekretnine", payload: "Iznajmljivanje" })}>
                <span>{t("Iznajmljivanje")}</span>
              </li>
            </ul>
          </div>
        ) : (
          <div>
            ...
            <span className="show-result">
              {t("Prikazivanje")}{" "}
              <span>
                {(title === "Agency Listing" ? 9 : 6) * (grid?.toPage || 1) - (title === "Agency Listing" ? 9 : 6)}-{(title === "Agency Listing" ? 9 : 6) * (grid?.toPage || 1) > productCount ? productCount : (title === "Agency Listing" ? 9 : 6) * (grid?.toPage || 1)}  {productCount}
              </span>{" "}
               {t("nekretnina")}
            </span>
          </div>
            ...
            {(AdvancedSearchShow || mobileSize) && (
              <div className="filter-bottom-title">
                <h6 className="mb-0 font-roboto" onClick={() => setAdvancedSearchOpen(!advancedSearchOpen)}>
                  {t("Dodatna pretraga")} <AlignCenter className="float-end ms-2" />
                </h6>
              </div>
            )}
          </li>
          ...
          <li>
            <Dropdown isOpen={isOpen} toggle={() => setIsOpen(!isOpen)}>
              <DropdownToggle className=" font-rubik">
                <span>{t(sortirajPrema) || t("Datum - najnoviji")}</span> <i className="fas fa-angle-down ms-lg-3 ms-2"></i>
              </DropdownToggle>
              <DropdownMenu className=" text-start">
                <DropdownItem onClick={() => dispatch({ type: "sortirajPrema", payload: "Datum - Najnoviji" })}>{t("Datum - najnoviji")}</DropdownItem>
                <DropdownItem onClick={() => dispatch({ type: "sortirajPrema", payload: "Datum - Najstariji" })}>{t("Datum - najstariji")}</DropdownItem>
                <DropdownItem onClick={() => dispatch({ type: "sortirajPrema", payload: "Cijena - Od više ka nižoj" })}>{t("Cijena - od više ka nižoj")}</DropdownItem>
                <DropdownItem onClick={() => dispatch({ type: "sortirajPrema", payload: "Cijena - Od niže ka višoj" })}>{t("Cijena - od niže ka višoj")}</DropdownItem>
              </DropdownMenu>
            </Dropdown>
          </li>
          {gridBar && (
